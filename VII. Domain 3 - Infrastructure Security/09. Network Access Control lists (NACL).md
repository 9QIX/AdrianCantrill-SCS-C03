...
This lesson explains Network ACLs as an infrastructure-security control. It converts the source lesson into the architecture, decision points, and failure modes most relevant to AWS Certified Security – Specialty (SCS-C03).
...
- **Core Skill:** Explain, configure, and troubleshoot Network ACLs without confusing reachability with authorization.

## Core Concepts

- A network access control list applies at subnet boundaries and contains separate inbound and outbound rules.
- Rules are processed by ascending rule number; the first match wins, and an unmatched packet is denied.
- Network ACLs support allow and deny entries but cannot reference security groups.
- Because they are stateless, response traffic and ephemeral client ports must be allowed explicitly.

## How It Works

1. A network access control list applies at subnet boundaries and contains separate inbound and outbound rules.
2. Rules are processed by ascending rule number; the first match wins, and an unmatched packet is denied.
3. Network ACLs support allow and deny entries but cannot reference security groups.
4. Because they are stateless, response traffic and ephemeral client ports must be allowed explicitly.
...
> **NACL: numbered, subnet-level, allow-or-deny, and stateless.**
...
1. What security problem does Network ACLs solve?

<details>
<summary>Answer</summary>

It provides the control described by the lesson: a network access control list applies at subnet boundaries and contains separate inbound and outbound rules.
...
NACL: numbered, subnet-level, allow-or-deny, and stateless. The surrounding route, policy, and filtering dependencies must also be evaluated.
...
- https://learn.cantrill.io/courses/1723753/lectures/39153006
