# Policy Interpretation: Time-Bounded S3 Access

## Overview

This example overlays an Allow and a conditional Deny for objects in a holiday-gifts bucket. It demonstrates statement decomposition, S3 object ARNs, date conditions, and explicit-deny precedence.
...
- **Core Skill:** Evaluate overlapping conditional IAM statements accurately.

## Core Concepts

- The Allow grants PutObject, PutObjectAcl, GetObject, GetObjectAcl, and DeleteObject on every object under the bucket ARN ending /*.
- The Deny overlaps only GetObject and GetObjectAcl on the same object resources.
- DateGreaterThan and DateLessThan conditions on aws:CurrentTime activate the Deny between the specified UTC instants.
- During that window the identity can write and delete but cannot read; outside it, the original Allow remains effective.

## How It Works

1. Separate the policy into its two statements.
2. Map each action to the object ARN and identify the read-action overlap.
3. Evaluate both date conditions against aws:CurrentTime.
4. Apply the conditional explicit Deny before reporting the final access result.
...
> **Overlay statements: when the clock activates Deny, read access disappears.**
...
- DateGreaterThan and DateLessThan conditions on aws:CurrentTime activate the Deny between the specified UTC instants.
- During that window the identity can write and delete but cannot read; outside it, the original Allow remains effective.
- Exam focus: Evaluate overlapping conditional IAM statements accurately.
...
- https://learn.cantrill.io/courses/1723753/lectures/39152948
