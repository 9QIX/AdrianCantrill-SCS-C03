...
Policy variables substitute request-context values into eligible string elements, enabling reusable user-, tag-, or session-aware policies. Variables are powerful only when the referenced context key is reliably present.
...
- **Core Skill:** Design scalable attribute-based permissions without unsafe assumptions.

## Core Concepts

- Variables use the ${context-key} syntax and can appear in Resource and supported Condition values, not as arbitrary JSON substitution.
- Common examples include ${aws:username}, principal tags, resource tags, and request tags.
- If a single-valued variable has no value, the statement may not match; Null checks and set operators require deliberate handling.
- Use stable attributes for federation because aws:username is not universally available to role sessions.

## How It Works

1. Identify the variable and the request context key that supplies it.
2. Determine whether that key exists for every expected principal and request path.
3. Substitute the value and evaluate the resulting resource or condition.
4. Test missing, multiple, malformed, and adversarial values before deployment.
...
> **A policy variable is request-time substitution, not a stored permission.**
...
- If a single-valued variable has no value, the statement may not match; Null checks and set operators require deliberate handling.
- Use stable attributes for federation because aws:username is not universally available to role sessions.
- Exam focus: Design scalable attribute-based permissions without unsafe assumptions.
...
- https://learn.cantrill.io/courses/1723753/lectures/39152947
