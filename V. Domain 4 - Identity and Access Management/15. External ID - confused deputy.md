...
An external ID is a customer-specific value used in a third-party role trust policy to reduce cross-account confused-deputy risk. The third party supplies it when calling AssumeRole, proving which customer context it is acting for.
...
- **Core Skill:** Prevent a SaaS provider from accidentally using one customer role for another customer.

## Core Concepts

- The third-party provider, not the customer, should generate a unique and non-secret external ID for each customer.
- The role trust policy tests sts:ExternalId; the provider must pass the matching value to STS.
- External ID does not replace trust in the provider account, least privilege, logging, or MFA where applicable.
- Use aws:SourceArn and aws:SourceAccount for AWS service confused-deputy scenarios when the service supports them; ExternalId is the third-party AssumeRole pattern.

## How It Works

1. The customer creates a role trusting the provider account and requiring a unique ExternalId.
2. The provider maps the customer request to that value and calls AssumeRole.
3. STS evaluates both the trusted principal and condition.
4. Only the correctly scoped provider request receives role credentials.
...
> **Third party plus unique customer ExternalId equals scoped delegation.**
...
- External ID does not replace trust in the provider account, least privilege, logging, or MFA where applicable.
- Use aws:SourceArn and aws:SourceAccount for AWS service confused-deputy scenarios when the service supports them; ExternalId is the third-party AssumeRole pattern.
- Exam focus: Prevent a SaaS provider from accidentally using one customer role for another customer.
...
- https://learn.cantrill.io/courses/1723753/lectures/39152956
