...
Identity-based policies are JSON permission documents attached to IAM users, groups, or roles. This chapter develops a reliable method for reading statements and predicting authorization outcomes.

## Exam Alignment

- **Primary SCS-C03 Domain:** Identity and Access Management
- **Secondary Domain(s):** Security Foundations and Governance
- **Main AWS Services:** AWS Identity and Access Management (IAM)
- **Lesson Type:** Policy Analysis
- **Exam Importance:** High
- **Core Skill:** Interpret identity policies and calculate effective permissions.

## Core Concepts

- A policy document contains Version and one or more Statement objects.
- Each statement combines Effect, Action, Resource, and optional Condition; Principal is normally omitted from identity policies because the attachment identifies the principal.
- Action and Resource must match the request, including whether the service distinguishes a container ARN from a child-resource ARN.
- Managed policies are reusable; inline policies have a one-to-one relationship with their identity.

## How It Works

1. Start with the implicit deny and identify the authenticated principal.
2. Inventory every statement and determine which actions, resources, and conditions match the request.
3. Combine applicable identity, resource, boundary, session, and organization policies.
4. Return Allow only when an applicable allow remains and no explicit deny applies.
...
> **Match the request, find an allow, then search for a deny.**
...
- Action and Resource must match the request, including whether the service distinguishes a container ARN from a child-resource ARN.
- Managed policies are reusable; inline policies have a one-to-one relationship with their identity.
- Exam focus: Interpret identity policies and calculate effective permissions.
...
- https://learn.cantrill.io/courses/1723753/lectures/39152925
