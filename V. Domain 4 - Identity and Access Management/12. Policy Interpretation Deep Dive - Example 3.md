# Policy Interpretation: Per-User Amazon S3 Home Folders

## Overview

This example builds console and object permissions for per-user S3 prefixes. It highlights bucket-level versus object-level actions, ListBucket prefix conditions, and policy variables.

## Exam Alignment

- **Primary SCS-C03 Domain:** Identity and Access Management
- **Secondary Domain(s):** Data Protection
- **Main AWS Services:** AWS IAM, Amazon S3
- **Lesson Type:** Policy Analysis
- **Exam Importance:** High
- **Core Skill:** Build and interpret least-privilege prefix-based S3 access.

## Core Concepts

- ListAllMyBuckets and GetBucketLocation support console navigation and generally use Resource * because of action-level resource support.
- ListBucket targets the bucket ARN and can restrict visible keys with the s3:prefix condition.
- Object actions target an object ARN such as arn:aws:s3:::example-bucket/home/${aws:username}/*.
- The user-name variable produces separation only for IAM users with expected names; federated designs should use suitable session attributes or principal tags.

## How It Works

1. Separate global navigation, bucket listing, and object-data statements.
2. Match bucket actions to the bucket ARN and object actions to the /* object ARN.
3. Substitute the current principal attribute into the prefix.
4. Verify both list visibility and direct object requests because hiding a key from listing is not itself an object-access deny.
...
> **List the bucket ARN; access the object ARN; scope both to the home prefix.**
...
- Object actions target an object ARN such as arn:aws:s3:::example-bucket/home/${aws:username}/*.
- The user-name variable produces separation only for IAM users with expected names; federated designs should use suitable session attributes or principal tags.
- Exam focus: Build and interpret least-privilege prefix-based S3 access.
...
- https://learn.cantrill.io/courses/1723753/lectures/39152951
