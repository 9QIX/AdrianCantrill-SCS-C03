...
AWS authorization evaluates identity policies, resource policies, organization guardrails, permissions boundaries, and session policies in context. Correct exam reasoning requires identifying grants, limits, and explicit denies rather than treating policies as additive lists.

## Exam Alignment

- **Primary SCS-C03 Domain:** Identity and Access Management
- **Secondary Domain(s):** Security Foundations and Governance
- **Main AWS Services:** AWS IAM, AWS Organizations
- **Lesson Type:** Policy Analysis
- **Exam Importance:** High
- **Core Skill:** Calculate effective permissions across policy types.

## Core Concepts

- Requests begin implicitly denied; an applicable allow is required.
- Identity- and resource-based policies can grant access, subject to principal type, account boundary, and service behavior.
- Service control policies and resource control policies constrain permissions in organization scope; permissions boundaries constrain IAM identity permissions.
- Session policies further reduce a session, and every applicable explicit deny overrides allows.

## How It Works

1. Authenticate the principal and build the request context.
2. Collect policies that apply to the principal, action, resource, account, and organization.
3. Evaluate grants and the intersections imposed by boundaries, session policies, and organization controls.
4. Apply service-specific rules and return Deny if no allow remains or any explicit deny matches.
...
> **Grants open doors; guardrails shrink the hallway; explicit deny locks it.**
...
- Service control policies and resource control policies constrain permissions in organization scope; permissions boundaries constrain IAM identity permissions.
- Session policies further reduce a session, and every applicable explicit deny overrides allows.
- Exam focus: Calculate effective permissions across policy types.
...
- https://learn.cantrill.io/courses/1723753/lectures/39152952
