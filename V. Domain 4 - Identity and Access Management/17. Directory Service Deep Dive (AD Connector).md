...
AD Connector is a directory gateway that redirects compatible AWS authentication requests to an existing self-managed Microsoft Active Directory. It does not cache directory data or create a separate directory domain.
...
- **Core Skill:** Select AD Connector only for proxy-style use of an existing directory.

## Core Concepts

- AD Connector relies on network and DNS connectivity to existing domain controllers.
- Authentication is performed against the existing directory, so directory availability and password policies remain authoritative.
- Small and large sizes describe proxy capacity, not a managed directory edition.
- Use it when workloads must retain the existing directory as the source of truth and trust-based AWS Managed Microsoft AD is unnecessary.

## How It Works

1. Deploy connectors in two subnets across Availability Zones.
2. Configure DNS server addresses and a delegated service account in the existing directory.
3. Allow required network paths between connectors and domain controllers.
4. Point supported AWS applications to the connector and test authentication and failover.
...
> **AD Connector forwards authentication; it does not host Active Directory.**
...
- Small and large sizes describe proxy capacity, not a managed directory edition.
- Use it when workloads must retain the existing directory as the source of truth and trust-based AWS Managed Microsoft AD is unnecessary.
- Exam focus: Select AD Connector only for proxy-style use of an existing directory.
...
- https://learn.cantrill.io/courses/1723753/lectures/39152960
