# Service-Linked Roles and iam:PassRole

## Overview

Service-linked roles are predefined roles linked directly to an AWS service. The iam:PassRole permission allows a principal to configure a service with a role, which is security-sensitive because the service then exercises that role.
...
- **Core Skill:** Control delegated service permissions and distinguish role passing from role assumption.

## Core Concepts

- A service-linked role has a service-defined trust relationship and permissions tailored to the linked service.
- The service commonly creates and manages the role, and deletion may require removing dependent resources first.
- iam:PassRole is checked when a caller passes a role to a service; it is not an API call and therefore has no standalone CloudTrail event.
- Restrict PassRole by role ARN and, where useful, iam:PassedToService or iam:AssociatedResourceArn conditions.

## How It Works

1. A caller creates or updates a resource and names an IAM role.
2. AWS verifies the caller can pass that role to the destination service and usually that both are in the same account.
3. The service assumes the role through its trust policy.
4. The service performs actions allowed by the role permissions policy.
...
> **PassRole lets a service wear a role; it does not let the caller wear it.**
...
- iam:PassRole is checked when a caller passes a role to a service; it is not an API call and therefore has no standalone CloudTrail event.
- Restrict PassRole by role ARN and, where useful, iam:PassedToService or iam:AssociatedResourceArn conditions.
- Exam focus: Control delegated service permissions and distinguish role passing from role assumption.
...
- https://learn.cantrill.io/courses/1723753/lectures/39152934
