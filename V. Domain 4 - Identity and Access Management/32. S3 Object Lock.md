...
S3 Object Lock protects individual object versions with write-once-read-many controls. Retention periods and legal holds support accidental-deletion resistance, ransomware resilience, and regulated records retention.
...
- **Core Skill:** Select and reason about immutable S3 retention controls.

## Core Concepts

- Object Lock works only with S3 Versioning and protects a particular version, not the object key as an abstract name.
- Governance mode can be bypassed only with s3:BypassGovernanceRetention and an explicit bypass request.
- Compliance mode cannot be shortened or bypassed by any principal, including the account root user, during retention.
- A legal hold has no expiration and remains until a principal with permission removes it; it is independent of retention mode.

## How It Works

1. Enable Object Lock on the bucket and establish default retention if required.
2. Write an object version and apply governance or compliance retention, a legal hold, or both.
3. S3 evaluates any permanent-delete or retention-change request against that version protection.
4. A simple delete can add a delete marker, but the protected version remains preserved.
...
> **Governance has a privileged bypass; compliance waits; legal hold has no clock.**
...
- Compliance mode cannot be shortened or bypassed by any principal, including the account root user, during retention.
- A legal hold has no expiration and remains until a principal with permission removes it; it is independent of retention mode.
- Exam focus: Select and reason about immutable S3 retention controls.
...
- https://learn.cantrill.io/courses/1723753/lectures/39152976
