# IAM Permissions Boundaries and Delegated Administration

## Overview

A permissions boundary sets the maximum identity-based permissions available to an IAM user or role. It enables controlled delegation: an administrator may create identities and policies but cannot make those identities exceed the approved boundary.
...
- **Core Skill:** Design safe IAM administration delegation.

## Core Concepts

- A boundary is a managed policy attached as a limit, not a permission grant.
- Effective identity permissions are the intersection of identity-based allows and boundary allows, subject to other policies and explicit denies.
- Delegated creators must be forced to attach an approved boundary and prevented from changing or removing it.
- Resource-policy grants to certain principal types have nuanced interactions with boundaries, so principal ARN and session ARN distinctions matter.

## How It Works

1. Security administrators create and protect an approved boundary policy.
2. A delegated administrator receives permission to create roles or users only when the required boundary is attached.
3. The delegate attaches ordinary permissions policies inside that ceiling.
4. AWS evaluates the identity allow and boundary intersection on every request.
...
> **A boundary is a ceiling, never a floor and never a grant.**
...
- Delegated creators must be forced to attach an approved boundary and prevented from changing or removing it.
- Resource-policy grants to certain principal types have nuanced interactions with boundaries, so principal ARN and session ARN distinctions matter.
- Exam focus: Design safe IAM administration delegation.
...
- https://learn.cantrill.io/courses/1723753/lectures/39152929
