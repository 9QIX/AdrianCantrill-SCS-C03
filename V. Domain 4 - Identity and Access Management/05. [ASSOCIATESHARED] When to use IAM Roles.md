...
Roles are the standard mechanism for granting temporary AWS permissions to workloads, federated users, AWS services, and cross-account principals. They reduce the exposure and lifecycle burden of long-term keys.
...
- **Core Skill:** Select roles instead of embedded or long-term credentials.

## Core Concepts

- Use service roles for AWS services, instance or task roles for workloads, and cross-account roles for delegation.
- Use federation or IAM Identity Center roles for human workforce access.
- Role chaining can restrict session duration and carries session context into another assumption.
- A role does not remove the need for least privilege, trust-policy conditions, or monitoring.

## How It Works

1. Select the trust relationship for the workload, service, identity provider, or external account.
2. Attach only the permissions required by the assumed workload or person.
3. Obtain temporary credentials through the native integration or STS.
4. Monitor assumptions and refine permissions using observed access.
...
> **If an identity can borrow permissions, give it a role instead of a permanent key.**
...
- Role chaining can restrict session duration and carries session context into another assumption.
- A role does not remove the need for least privilege, trust-policy conditions, or monitoring.
- Exam focus: Select roles instead of embedded or long-term credentials.
...
- https://learn.cantrill.io/courses/1723753/lectures/39152933
